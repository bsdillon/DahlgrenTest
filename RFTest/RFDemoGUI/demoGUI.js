...
function initialize(dataFile, archiveFile)
{
  start(dataFile)

  exchangeScriptFile("archiveSourceFile", archiveFile)
  setTimeout(function(){waitForArchive(); },100);
}

function waitForArchive()
{
  if(!archive)
  {
    setTimeout(function(){waitForArchive(); },100);
  ...
  {
    var option = document.createElement("option");
    option.text = archive[i][1]+" "+archive[i][2];
    option.value = archive[i][0];
    selection.add(option);
  }
}
...
function updateDataFile()
{
  ...
  dataFile = selection.value;
}

function exchangeScriptFile(id, file)
{
  var currentData = document.getElementById(id);
  if(currentData)
  {
    currentData.remove();
  }
...
{
  //Find the selectData object
  dataFile = newFile;
  exchangeScriptFile("dataSourceFile", dataFile);
...
{
  var reportNames = document.getElementById("reportNames");
  reportNames.replaceChildren();
  ...
  for(var testIndex=0; testIndex<length+1; testIndex++)
  {
    ...
    { //Case where test run has been completed and test data is available
      ...
      li.innerHTML = rawData[testIndex];
