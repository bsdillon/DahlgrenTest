import math
from parts.schemata import Schemata, Source, Sink, Label

##############################################################################
# Stores the amount of connections between nodes (for communication diagrams)#
##############################################################################
class Sequence:
    def __init__(self, name:str, color:str, broken:bool=False):
        self.broken = broken
        self.name = name
        self.color = color
        self.BGColor = "DDDDDD"
        ...
        if len(self.color)==6:
            try:
                ...
                hsp = math.sqrt((0.299 * (rVal * rVal)) + (0.587 * (gVal * gVal)) + (0.294 * (bVal * bVal)) )
                if (hsp>127.5):
                    self.FColor = "FFFFFF"
                else:
                    self.FColor = "000000"
...
class Relationships:
    ...
    def ValidateSchema():
        ...
        if Relationships.SenderIndex==-1 or Relationships.ReceiverIndex == -1 or Relationships.LabelIndex == -1:
            raise Exception("Invalid Relationship Schema: must have Source, Sink, and Label in some position")

    def __init__(self, data:[]):

        if len(data) != Schemata.schemaLength:
            raise Exception("Invalid Relationship: must match schema cardinality  "+str(len(data))+"<>"+str(Schemata.getSchemaLength()))
        for i in range(Schemata.getSchemaLength()):
            if len(data[i])==0 and not (i==Relationships.SenderIndex or i==Relationships.ReceiverIndex or i==Relationships.LabelIndex):
                pass #we allow empty non-critical values
            elif not Schemata.validateValue(i,data[i]):
                raise Exception("Invalid Relationship: "+str(i)+"th element "+str(data[i])+" does not match schema type "+str(Schemata.getNthSchemaTitle(i)))
    ...
    def getParamAt(self, index:int):
        if index < len(self.data):
            return self.data[index]
        raise Exception("Parameter index "+index+" exceeds current relationship")

    def getParam(self, name:str):
        index = Schemata.getTitleIndex(name)
        if index == -1:
            raise Exception("Parameter name "+name+" is unknown in schema")

        return self.data[index]

    def validateParam(self, name:str, value:str):
        index = Schemata.getTitleIndex(name)
        if index == -1:
            raise Exception("Parameter name "+name+" is unknown in schema")

        if not Schemata.validateValue(i,value):
            #invalid value for this data type
            #this should fail loudly and as soon as possible
            raise Exception("Invalid Parameter: "+name+": "+value+" does not match schema type "+str(Schemata.getNthSchemaTitle(i)))

        if not value==self.data[i]:
            #values do not match
            return False
    ...
    def set_sequence(self, seq:Sequence):
        if self.sequence:
            ...
            print("next sequence "+str(seq))
            raise Exception("Sequence already set; no relationship can be a member of TWO or more")
        self.sequence = seq
        seq.addMember(self)
