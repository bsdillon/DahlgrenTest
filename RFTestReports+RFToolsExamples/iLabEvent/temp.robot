...
Log Keyword
   [Arguments]    ${value}
   Log    ${value}
   Record Test Case    ${TEST NAME}    This is quite ridiculous    ${value}    --
   Record Test Case    --    Something silly    ${value}    --
   Record Test Case    Boring    --    ${value}    --
   Record Test Case    ${Test Name}    Log Keyword    ${value}    --
   [TEARDOWN]    Record Test Case    ${TEST NAME}    Setup OQE    Pass    ${value}
