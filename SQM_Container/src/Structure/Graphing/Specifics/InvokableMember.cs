...
namespace SoftwareAnalyzer2.Structure.Graphing.Specifics
{
    ...
    {
        private static Boolean verboseMatch = false; //Allow a human to make the final selection in an unknown case. Rare.
        ...
        public static DefinedMember GetBestMatch(INavigable unknownMethod, GraphNode parameterScope, List<InvokableMember> candidates, List<GraphNode> paramTypes, List<INavigable> myParams)
        {
            ...
            if (verboseMatch)
            {
                lock (MatcherLock)
                {
                    //This has never been been used BSD as of 12/2024
                    //Likely never will. It gives the human the option
                    //to select which method is a match.
                    Console.WriteLine("Still can't identify correct method "+unknownMethod);
                    return finalMatches[0];
