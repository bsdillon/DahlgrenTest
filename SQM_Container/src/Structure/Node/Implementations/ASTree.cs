...
namespace SoftwareAnalyzer2.Structure.Node.Implementations
{
    ...
    public class ASTree : IModifiable
    {
        ...
        {
            logger = CrossCutting.GetLogger(typeof(ASTree));
        ...
        {
            if (!children.Contains((ASTree)child))
            {
        ...
        public void ReplaceCode(string codeSnippet)
        {
            int myLineStart = lineStart;
            int myCharStart = charStart;
            ClearCode(ClearCodeOptions.KeepLine);
            AddCode(codeSnippet, myLineStart, myCharStart);
        }
        ...
        {
            //TODO needs to be moved to ANTLRTool; only used by Java parsing.
            ASTree a = (ASTree)node;
            logger.LogTrace($"Modifying type declaration for {node}");
            StringBuilder sb = new StringBuilder();
            ...
            IModifiable identifier = (IModifiable)node.GetFirstSingleLayer("identifier");
            logger.LogDebug($"Found identifier: {identifier}");
            this.CopyCode(identifier);
            ...
            subType.SetNode((Members)Enum.Parse(typeof(Members), a.Code.ToUpper()));
