...
class TestSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Relevant Lab groups need their cut too.
...
class LabSerializer(serializers.ModelSerializer):
    tests = TestSerializer(many=True, required=False, write_only = True)
    # May want to remove write_only?
    # It obfuscates COMPLETELY, so you can't preview tests, in case that's a desirable outcome...
    ...
    # Also delete the groups if the Lab goes.
    # See the relevant "destroy" function in views.py.
    # Note that that will not be utilized when deleting via the admin page, so avoid doing that.
