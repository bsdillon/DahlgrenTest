    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for perm in get_perms_for_model(Lab):
                objects = get_objects_for_user(user, "holisticPrototype." + perm.codename, accept_global_perms=False, use_groups=False)
                if objects:
                    self.stdout.write(self.style.SUCCESS(str(perm)))
                    for obj in objects:
            ...
            for group in user.groups.all():
                ...
                for perm in Permission.objects.filter(group=group):
                    self.stdout.write(self.style.SUCCESS(str(perm)))
                for perm in get_perms_for_model(Lab):
                    objects = get_objects_for_group(group, "holisticPrototype." + perm.codename, accept_global_perms=False)
                    if objects:
                        self.stdout.write(self.style.SUCCESS(str(perm)))
                        for obj in objects:
                            self.stdout.write(self.style.SUCCESS(str(obj)))
