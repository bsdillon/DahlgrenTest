...
from Parsers.parseClass import ParseClass  # please ensure module parseMessage.py is in same directory
...
class MessageTracker:
    ...
    def __init__(self):
        ...
        self.unique_message_list = []  # a list of all unique messages-class-names. messages with multiple topics only appear once.
        self.all_message_list = []  # an ordered list of all messages-class-names. if messages have more than one topic,
                                    # they appear for each topic.
        self.topic_list = []  # a list of all topics. this list is correlated by index with the all_message_list
        self.topic_messagedict = {}  # a mapping of topics : message-class-names. Will be empty until create_dicts() is called
        self.AllHeaderFiles = {}
        self.driverDirectory = ""
    # end __init__

    def AddMessage(self, message, topic): //adds a list of different messages and only wants new messsage
        """Adds a message-topic pair to the MessageTracker. These items MAY confirm a candidate message class"""
    ...
    def ValidateDrivers(self, fileList, driverPath): //checks if the drivers are missing
        """
        Searches for exact correlation of expected messages with those found in driver folder
        ...
        for message in self.unique_message_list:
            if not message + self.DriverFileEnd in fileList:
                return False
        print 'Driver files match with expected messages'

        print 'Reading driver files...',
        ...
        for pair in topicMap:
            ...
            if pair == '\n':
                continue
            count += 1 //add classes and associates it
            if count % 50 == 0:
                print '.',
    ...
    def AddNonCandidate(self, path, file): 
        self.AllHeaderFiles[file] = [path,file] //find path to file 
    # end AddNonClassFile

    def AddCandidate(self, name, path, file):
        """Adds a Candidate Class to the the MessageTracker. Until confirmed this class MAY be a message"""
        if name not in self.candidate_dict.keys():
            self.candidate_dict[name] = [path,file]
            self.AllHeaderFiles[file] = [path,file]
    # end AddCandidate

    def CreateAll(self, driver_dir): //knows directory
        """
        This method performs final correlation on the message-topic pairs and candidate classes
        before executing the driver creation
        Precondition: All candidate classes and message-topic pairs have been added
        :param driver_dir: location for all drivers
        :return: None
        """
        self.driverDirectory = driver_dir

        print '{candidate_count} Candidate classes identified\n'.format(candidate_count = len(self.candidate_dict.keys()))
        ...
        if self.unfound_list:
            print '{unfound_list_count} messages were not found:\n'.format(unfound_list_count = len(self.unfound_list))
        ...
        #Go through all verified header files, create a PathList object for each one, and append
        ...
        for key, value in self.accepted_dict.items():
            ...
            p = ParseClass(value[0], value[1], driver_dir, key + self.DriverFileEnd, True)
    ...
    def WriteConfigFile(self):
        ...
        for topic, message in self.topic_messagedict:
            config.write('{topic}{mapDelim}{message}{elementDelim}'.format(topic = topic, mapDelim = MessageTracker.mapSeparator, message = message, elementDelim = MessageTracker.elementSeparator))
        ...
        for f in self.AllHeaderFiles:
            ...
            config.write('{f},{v_0},{v_1}{elementDelim}'.format(f = f, v_0 = v[0], v_1 = v[1], elementDelim = MessageTracker.elementSeparator))
        print 'Done\n'
        config.close()
    # end WriteConfigFile

    def ProcessCodeBase(self):
        dependencies = DependencyTracker(self.driverList, self.AllHeaderFiles, self.driverDirectory)

        #write all driver files
        dependencies.WriteFiles(self.driverDirectory)
        dependencies.WriteFactory(self.driverDirectory, sorted(self.topic_messagedict), self.driverList)
    #end ParseCodeBase