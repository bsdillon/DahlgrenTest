...
class DependencyFinder:
    def __init__(self, path, filename, driver_path):
        ...
        try:
            self.parser = CppHeaderParser.CppHeader(
    ...
    def AppendTypes(self, typeList):
        ...
        for c in self.parser.classes.keys():
            ...
            for t in self.parser.classes[c]._public_typedefs:
                name = TypeDictionary.AddTypeName(t, self.parser.classes[
                    c]._public_typedefs[t])
                typeList[name.GetName()] = self
    ...
    def ParseType(self, name):
        for t in self.parser.typedefs.keys():
            ...
            if p.GetName() == name:
                return TypeDef(self.path, self.filename, self.driver_path,
                               p.GetName(), p.GetName())
        ...
        for c in self.parser.classes.keys():
            ...
            for e in self.parser.classes[c]['enums']['public']:
                en = e['name']
                if en == name:
                    pe = ParseEnum(self.path, self.filename, self.driver_path,
                                   en, en)
                    pe.SetClass(c)
                    return pe
        ...
        for e in self.parser.enums:
            en = e['name']
            if en == name:
                p = ParseEnum(self.path, self.filename, self.driver_path, en,
                              en)
                return p
    # end ParseType
