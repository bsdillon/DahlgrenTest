...
class ParseClass(AbstractParser):
    """
    Track one message's path and filename and the associated driver's path and filename.
    ...
    def ToFile(self):
        return '{hdr_path}{delim}{hdr_filename}{delim}{driver_path}{delim}{driver_filename}\n'.format(hdr_path = self.header_path,
                                                                                                      hdr_filename = self.header_filename,
                                                                                                      driver_path = self.driver_path,
                                                                                                      driver_filename = self.driver_filename,
                                                                                                      delim = ParseClass.fileDelimeter)

    def __init__(self, header_path, header_filename, driver_path, driver_filename, original, target = ''):
        ...
        if driver_filename[len(driver_filename)-9:] == '_Driver.h':
            df = driver_filename
        self.name = df[:-9]
        AbstractParser.__init__(self, header_path, header_filename, driver_path, df, target)

    def Parse(self):
        if len(self.parser.classes) > 1:
            #files with multiple classes are possible IFF we are designating a target
            if not self.target == '':
    ...
    def ParseParentClass(self):
        if len(self.myObject['inherits']) > 0:
            #only update parent class if it is the Message_T. Otherwise we create a new factory method
    ...
    def ParseMethods(self):
        for m in self.myObject['properties']['public']:
            if m['name'] == 'resourceKey':
                sdflkjdsf = 0
            self.Methods.append(Method(self.header_full, m['line_number'], m['raw_type'], m['name']))
    ...
    def WriteIncludesOnly(self, f, dependencies, includeList):
        """
        This method is the recursive call for parent class to add include statements
        :param f:
        :param dependencies:
        :param includeList:
        :return:
        ...
        if self.parentClassName:
            dependencies[self.parentClassName].WriteIncludesOnly(f, dependencies, includeList)

        for m in self.Methods:
            ri = '"{return_import}"'.format(return_import = m.GetReturnImport(dependencies))
    ...
    def WriteMethodsOnly(self, f, dependencies):
        ...
        """
        if self.parentClassName:
            dependencies[self.parentClassName].WriteMethodsOnly(f, dependencies)
    ...
    def GetAllMethods(self, count, methodGroups, methodList, dependencies):
        ...
        if self.parentClassName:
            count = dependencies[self.parentClassName].GetAllMethods(count, methodGroups, methodList, dependencies)
    ...
    def WriteHeader(self, dependencies):
        ...
        if len(self.myObject['namespace']) > 0:
            if not self.myObject['namespace'][-2:] == '::':
                fullSourceName = '{ns}::{target}'.format(ns = self.myObject['namespace'], target = self.target)
            else:
        ...
        includeList.append('"{header_filename}"'.format(header_filename = self.header_filename))
        ...
        f.write(('class {target}_Driver: public virtual AbstractDriver\n'
                 ...
                 '{__}{__}bool factory;\n'
                 '\n'
                 '{__}{__}{fullSourceName} source;\n\n').format(__ = AbstractParser.space, target = self.target, fullSourceName = fullSourceName))
        ...
        f.write(('{__}public:\n'
                 ...
                 '\n'
                 '{__}{__}std::unique_ptr<AbstractDriver> CreateDriver({fullSourceName} p) {{\n'
                 '{__}{__}{__}if (!factory) {{\n'
                 '{__}{__}{__}{__}throw DriverException("CreateDriver() is only valid from factory instances of {target}_Driver", DriverException::Level::UnstableProgram);\n'
                 '{__}{__}{__}}}\n'
                 '{__}{__}{__}return std::move(std::unique_ptr<AbstractDriver>(new {target}_Driver(p)));\n'
                 '{__}{__}}}\n\n').format(__ = AbstractParser.space, target = self.target, fullSourceName = fullSourceName))

        #only original Drivers need to have a conversion method which overrides the virtual one.
        if self.IsOriginal:
            f.write(('{__}{__}std::unique_ptr<AbstractDriver> CreateDriver(const infrastructureservices::common::Message_T* p) {{\n'
                     '{__}{__}{__}if (!factory) {{\n'
                     '{__}{__}{__}{__}throw DriverException("CreateDriver() is only valid from factory instances of {target}_Driver", DriverException::Level::UnstableProgram);\n'
                     '{__}{__}{__}}}\n'
                     '{__}{__}{__}{fullSourceName}* tmp = ({fullSourceName}*)p;\n'
        ...
        if self.parentClassName:
            mCount=dependencies[self.parentClassName].GetAllMethods(mCount, methodGroups, methodList, dependencies)
        ...
        f.write('{__}{__}{__}signatures = std::vector<AbstractDriver::ReturnTypes>({mCount});\n'.format(__ = AbstractParser.space, mCount = mCount))

        index = 0
        for m in methodList:
            rt = m.GetReturnAbstractType(dependencies)
            t = ParseClass.FormatReturnType(rt)
            f.write('{__}{__}{__}signatures[{index}] = {type};\n'.format(__ = AbstractParser.space, index = index, type = t))
            index += 1
        ...
        f.write(('\n'
                 ...
                 '{__}{__}}}\n'
                 '\n'
                 '{__}{__}~{target}_Driver() {{ }}\n\n').format(__ = AbstractParser.space, mCount = mCount, target = self.target))

        if self.IsOriginal:
            f.write(('{__}{__}infrastructureservices::common::Message_T* CreateRandom() {{\n'
                     '{__}{__}{__}if (!factory) {{\n'
                     '{__}{__}{__}{__}throw DriverException("CreateDriver() is only valid from factory instances of {target}_Driver", DriverException::Level::UnstableProgram);\n'
                     '{__}{__}{__}}}\n'
                     '\n'
        ...
        for m in methodList:
            ...
            if not size == 0:
                f.write(('{__}{__}{__}{__}case {mCount_minus_1}:\n'
                         '{__}{__}{__}{__}{__}return (int){size};\n').format(__ = AbstractParser.space, mCount_minus_1 = mCount - 1, size = size))
        f.write(('{__}{__}{__}}}\n'
                 ...
                 '{__}{__}}}\n\n').format(__ = AbstractParser.space, target = self.target))
        ...
        for rt in methodGroups:
            ...
            f.write(('{__}{__}{returnType} get{t}Method(int index) {{\n'
                     '{__}{__}{__}switch (index) {{\n').format(__ = AbstractParser.space, returnType = returnType, t = t))
            for count in methodGroups[rt].keys():
                m=methodGroups[rt][count]
                if "std::unique_ptr" in returnType:
                    returnCall = 'return std::move({mGetCall})'.format(mGetCall = m.GetCall())
                else:
                    returnCall = 'return {mGetCall}'.format(mGetCall = m.GetCall())
                f.write(('{__}{__}{__}{__}case {count}:\n'
                         '{__}{__}{__}{__}{__}{returnCall};\n').format(__ = AbstractParser.space, count = count, returnCall = returnCall))

            f.write(('{__}{__}{__}}}\n'
                     '{__}{__}{__}std::string msg = "{target}_Driver has no {t} method at index ";\n'
                     '{__}{__}{__}msg.append(std::to_string(index));\n'
                     '{__}{__}{__}throw DriverException(msg, DriverException::Level::DataLoss);\n'
                     '{__}{__}}}\n\n').format(__ = AbstractParser.space, target = self.target, t = t))
        ...
        for m in methodList:
            f.write(('{__}{__}{__}{__}case {mCount}:\n'
                     '{__}{__}{__}{__}{__}return {mWriteName};\n').format(__ = AbstractParser.space, mCount = mCount, mWriteName = m.WriteName()))
            mCount += 1
    ...
    def GetMethodSignatures(self, types, dependencies):
        ...
        if self.parentClassName:
            dependencies[self.parentClassName].GetMethodSignatures(types, dependencies)
    ...
    def FormatCodeReturnType(rt):
        ...
        if rt == 'char*':
            ...
            returnDict['returnType'] = 'std::unique_ptr<std::unique_ptr<AbstractEnum>[]>'
            ...
            returnDict['arrayWithSize'] = 'std::unique_ptr<AbstractDriver>[size]'
            returnDict['returnType'] = 'std::unique_ptr<std::unique_ptr<AbstractDriver>[]>'
        else:
    ...
    def WriteAbstract(driver_dir, enumTypes):
        ...
        for e in enumTypes:
            f.write('{__}{__}{__}{returnType},\n'.format(__ = AbstractParser.space, returnType = ParseClass.FormatReturnType(e)))
        f.write(('{__}{__}}};\n'
                 '\n'
                 '{__}protected:\n'
        ...
        for e in enumTypes:
            f.write(('{__}{__}{__}{__}case {returnType}:\n'
                     ...
                     ).format(__ = AbstractParser.space, returnType = ParseClass.FormatReturnType(e), description = ParseClass.FormatDescription(e)))
        f.write(('{__}{__}{__}}}\n'
    ...
    def GetFileIncludes(self, file):
        file.write('#include "{driver_filename}"\n'.format(driver_filename = self.driver_filename))

    def GetValue(self, source, forArray):
        if forArray:
            return 'new {getType}({source}[i])'.format(getType = self.GetType(), source = source)
        return 'new {getType}({source})'.format(getType = self.GetType(), source = source)
