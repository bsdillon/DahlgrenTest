...
class ParseUnion(AbstractParser):
    def __init__(self, header_path, header_filename, driver_path, driver_filename, target=''):
        AbstractParser.__init__(self, header_path, header_filename, driver_path, driver_filename+ "_Driver.h", target)

    def Parse(self):
        for c in self.parser.classes:
            parts = c.split(' ')
            if len(parts) > 1 and parts[0] == 'union' and parts[1] == self.target:
