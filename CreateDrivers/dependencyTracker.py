...
class DependencyTracker:
    def __init__(self, driverlist, allHeaderFiles, driverDirectory):
        ...
        #this will run once and then again as necessary to eliminate any new dependencies
    ...
    def WriteFactory(self, driver_dir, topicMap, drivers):
        ...
        f.write(('\n'
                 ...
                 '{__}{__}{__}auto answer = std::vector<std::string>({topicMap_len});\n').format(__ = AbstractParser.space, topicMap_len = len(topicMap)))

        index=0
        for t in topicMap:
            f.write('{__}{__}{__}answer[{index}] = "{t_0}";\n'.format(__ = AbstractParser.space, index = index, t_0 = t[0]))
            index += 1
            count += 1
            if count % 20 == 0:
                print('.',end=" ")
        f.write(('\n'
                 ...
                 '{__}{__}static std::unique_ptr<AbstractDriver> GetTopicFactory(int index) {{\n'
        ...
        for t in topicMap:
            ...
            f.write(('{__}{__}{__}{__}case {index}:\n'
                     '{__}{__}{__}{__}{__}answer = std::unique_ptr<AbstractDriver>(new {driverName}());\n'
                     '{__}{__}{__}{__}{__}break;\n').format(__ = AbstractParser.space, index = index, driverName = driverName))
            index += 1
            count += 1
            if count % 20 == 0:
                print('.',end=" ")
