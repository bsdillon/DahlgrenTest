...
%----------------------------------------------------------------------------
\chapter*{Week of Dec 21 2020}
%
\textbf{MSCGen Revisited}\\\\
%
There is a Javascript port of MSCGen that does not break like the original download. The code is available on GitHub \url{https://github.com/mscgenjs}. Exploring this page, we can find a list of packages in \url{https://github.com/mscgenjs/mscgenjs-core}. Within this list, there is also a link to an online interpreter, a downloadable package that is compatible with Atom, a command line tool, an overview of incorporating MSCGen into HTML, and a list of features for the program. The GitHub page appears to be regularly updated by two listed users. 
%
The online interpreter includes a variety of examples, such as this simple Public Key Encryption chart:\\\\
%
\begin{center}
	\includegraphics[scale=0.9]{PKIExampleCode}
\end{center}
%
\begin{center}
	\includegraphics[scale=0.9]{PKIExample}
\end{center}
%
The link to the command line interface on the core page takes us here:\\ \url{https://www.npmjs.com/package/mscgenjs-cli},\\
where instructions for download and usage are provided. There is also a link to a tool called RunKit where the interface or other Node.js package can be tested. Once downloaded, the module can be imported in a few different ways listed on the page. It is recommended on the info page that the root module be used. The module is included with the download for Atom. Individual calls can be made to parse and render the steps under special circumstances such as the customization needed for this project. This can be done using the mscgen-js and mscgenjs-inpage script.  It is also possible to transpile between different formats such as msgenny, mscgen, xu, and json using the root module. It can also be used to export to dot and doxygen. \\\\
%
In addition to the links and information given, a readme page giving a basic overview of how MSCGen works is also given\\ \url{https://github.com/mscgenjs/mscgenjs-core/blob/master/doc/readme.md},\\
which could be useful when using or modifying MSCGen for our purposes. With the code available on GitHub, and several ways to use it MSCGen could be a useful tool. \\\\
%
Licensing could be an issue depending on how it is integrated, as it is not approved for use in non-free software projects. More information on licensing is listed on the core page at the bottom. The definition of the word non-free could be exploited based on specific usage. It is unclear how far this definition extends into the source of funding for any project where source code is incorporated. Definitely worth a look before considering this as a viable tool.\\\\
%
\chapter*{Week of December 14 2020}
\label{121520}
%
Eclipse supports JSON files, CSV files, XML/XMI schema, Java files. Some complications were discovered with UML Designer 8.1. This appears to be a known issue and, based on plug-in reviews, has been in and out of the marketplace:\\ \url{https://marketplace.eclipse.org/content/uml-designer#bootstrap-panel--2}\\\\
,but there are measures that can be taken to attempt a fix:\\\\ \url{https://github.com/ObeoNetwork/UML-Designer/issues/1062}. \\\\
%
After installing Eclipse Modeling Tools, you can explore the marketplace to find different UML tools. The first is UML Designer 8.1, which requires another pug-in, Sirius,to be installed. Sirius should be installed first. \\
%
After installing Sirius, there was an issue with installing the UML Designer 8.1 plug-in directly from the marketplace. One suggestion from GitHub was to uninstall Sirius, and attempt to install UML Designer first. Plug-ins that are required, but missing are automatically installed when another plug-in is dependent on them. This suggestion did not fix the problem. \\\\
%
Another potential fix is to download it directly from the project website \url{http://www.umldesigner.org/download/}. There are two options here. The first is to go to the "Help" tab in Eclipse. Here you can find the "Install New Software" option.\\\\ 
%
\includegraphics[scale=0.7]{InstallNewSoftware}\\\\
%
From here you can find the plug-in file that you downloaded from the project website and install it directly from your computer or the website, without going through the marketplace. \\\\
%
\includegraphics[scale=0.6]{AddSoftware}\\\\
%
The easiest way to add the software is to copy and paste the repository link from the UML Designer download page into the location field.\\\\
%
\includegraphics[scale=0.6]{RepositoryLink}\\\\
%
This method was also unsuccessful. Another method is to have UML Designer as a separate entity. By doing this, you are able to install and start up the program to explore. I have installed the updated version, UML Designer 9.0. There are some issues with having this kind of setup. I did explore the modeling example that was available and successfully loaded the package hierarchy diagram as outlined in the UML Designer tutorial page, but the sequence diagram never properly loaded. With the errors experienced during testing, I do not believe this to be a viable option at this time. The plug-in, and related projects are regularly updated so this may change in the future. More specific information will be provided on these errors in next weeks report.\\\\
%
Another tool listed in the given Excel file is Papyrus, which is also an Eclipse Project. The current version can be downloaded here: \url{https://www.eclipse.org/papyrus/download.html}. While it is related to Eclipse, it runs as a separate entity after download. 
%
To start, I experimented with the tutorials and example projects. The first is "Getting Started with UML2", which can be found by going to File $\rightarrow$ New $\rightarrow$ Example $\rightarrow$ UML2 Example Projects $\rightarrow$ Getting Started with UML2.\\\\
%
\includegraphics[scale=0.7]{GettingStartedExample}\\\\
%
This opens a file with Java code. When the program is run, it generates and saves a UML model for the project. The file for the model can be found in the Papyrus workspace folder. The PNG file of the model can be found in the project explorer on the left. \\\\
%
\includegraphics{ModelLocation}\\\\
%
By double-clicking the PNG file it will open in Papyrus for viewing.\\\\
%
\includegraphics[scale=0.8]{ExampleUMLModel}\\\\
%
Papyrus doesn't appear to be as flexible as it does not have its own marketplace, like the general Eclipse IDE, but it does have an option for creating a plug-in project which could be useful for modifying it to our own use. 
%
%---------------------------------------------------------------------------
