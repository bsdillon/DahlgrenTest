...
class ValueButtons(ttk.Frame):
    def __init__(self, parent, low:int, high:int, value:int=0, callback=None):
        ttk.Frame.__init__(self, parent)
        self.callback = callback
        self.low = low
        self.high = high
        if not((self.low<= value) and (value <= self.high)):
            raise ValueError(message=str(value)+" is invalid: must be in the range ["+self.low+", "+self.high+"]")
        self.value = value
    ...
    def changeValue(self, delta):
        ...
        if (self.low<= proposedValue) and (proposedValue <= self.high):
    ...
    def set(self, newValue):
        ...
        if not newValue == self.value:
            messagebox.showerror(title="Range Error", message=str(newValue)+" is invalid: must be in the range ["+self.low+", "+self.high+"]", parent=self)

    def resetRange(self, low:int, high:int, value:int):
        if not((self.low<= value) and (value <= self.high)):
            messagebox.showerror(title="Range Error", message=str(value)+" is invalid: must be in the range ["+self.low+", "+self.high+"]", parent=self)
        self.low = low
        self.high= high
        self.set(value)
